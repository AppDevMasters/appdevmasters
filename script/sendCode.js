...
async function sendCode() {
    ...
    try{
        let response = await fetch('https://client.taskit.pro/register/restore_pw/?email=' + emailInput.value, {
            method: 'get',
        });

        let result = await response.json();
        console.log(result)
        if(result.msg === 'successfully_reset'){
            console.log(result.msg)
            showAlert('Check email', 'alert-success');
            sleep(8);
            window.location.replace("https://client.taskit.pro/login/form");
        }else if (result.msg === "bad_email"){
            showAlert('Bad Email')
        }else{
